...
@app.get("/api/config")
def get_config(): return {"kakao_key": os.getenv("KAKAO_JS_KEY")}
...
def check_status(room_id: str):
    ...
    return { 
        ...
        "host_message": room.host_message 
    }

@app.post("/host-message/{room_id}")
def save_host_message(room_id: str, request: JoinRequest):
    if room_id not in rooms_db: return {"error": "No Room"}
    rooms_db[room_id].host_message = request.message
...
def reserve_slot(room_id: str, request: JoinRequest):
    ...
    # If full, create a new extra slot
    new_pos = len(room.slots)
    new_slot = Slot(position=new_pos, char="★", reserved_by=request.user_name)
    room.slots.append(new_slot)
    return {"status": "SUCCESS", "assigned_char": new_slot.char}
...
def join_room(room_id: str, request: JoinRequest):
    ...
    target_slot = None
    # 1. Check if user reserved a slot
    for slot in room.slots:
        if slot.reserved_by == request.user_name and not slot.is_filled:
            target_slot = slot; break
            
    # 2. If not reserved, find any empty slot
    if not target_slot:
        for slot in room.slots:
            if not slot.is_filled and slot.reserved_by is None:
                target_slot = slot; break
                
    # 3. If still no slot, create extra slot
    if not target_slot:
        new_pos = len(room.slots)
        target_slot = Slot(position=new_pos, char="★", reserved_by=request.user_name)
        room.slots.append(target_slot)
    ...
    if target_slot:
        ...
        try:
            ...
            print(f"저장 실패: {e}")
            return {"status": "ERROR", "message": "이미지 저장 실패"}
        return {"status": "SUCCESS"}
...
def make_card(room_id: str):
    ...
    # 베이지색 원단 배경 -> Light Gray to match index.html
    bg_color = (245, 245, 245) 
    dot_color = (224, 224, 224)
